...
- **Modulo richieste** → Worker con Turnstile + Cloudflare Email Service, più WhatsApp
...
Architettura: **Cloudflare Worker `viosadea`** (account Cloudflare dedicato), pagine
statiche + `/api/richiesta` che invia le richieste a **viosadea@gmail.com** con
Cloudflare Email Service, protetto da Turnstile.
...
4. [ ] Migrazione DNS da Netsons senza rompere la posta → [docs/dns-migration.md](docs/dns-migration.md)
5. [ ] Email Sending: `npm run cf:email:enable` e test del modulo
6. [ ] Dominio sul Worker (`routes` in `wrangler.jsonc`) + redirect apex → www
7. [ ] Repo GitHub + secrets per il deploy automatico
...
- [docs/setup-cloudflare.md](docs/setup-cloudflare.md) — primo setup: account, token, Turnstile, email, dominio, GitHub
